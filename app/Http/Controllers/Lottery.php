...
return redirect('/baseBonusSettings');
    ...
    public function lotteryResult(Request $request)
    {
        ...
        // گیمر لیست کیوری
        $players=DB::select("SELECT *,CRM.dbo.getCustomerPhoneNumbers(posId) as PhoneStr,CRM.dbo.getGameName(gameId) as GameName FROM (
            SELECT Row,Name,posId,PosName,gameId FROM (
            SELECT ROW_NUMBER() 
