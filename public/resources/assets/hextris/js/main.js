...
function showHelp() {
	if ($('#openSideBar').attr('src') == './images/btn_back.svg') {
		...
		$('#openSideBar').attr('src', baseUrl+"/resources/assets/hextris/images/btn_back.svg");
		if (gameState == 0 && gameState == -1 && gameState == 2) {
			$('#fork-ribbon').fadeIn(150, 'linear');
		}
	}

	$("#inst_main_body").html("<div id = 'instructions_head'> چطوری بازی کنم؟ </div><p>هدف هکستریس جلوگیری از خروج بلوک ها از داخل شش ضلعی خاکستری بیرونی است.</p><p>" + (settings.platform != 'mobile' ? 'کلیدهای جهت دار راست و چپ را فشار دهید' : 'روی سمت چپ و راست صفحه ضربه بزنید') + " برای چرخاندن شش ضلعی." + (settings.platform != 'mobile' ? ' برای سرعت بخشیدن به سقوط بلوک، فلش رو به پایین را فشار دهید' : '') + " </p><p>بلوک ها را پاک کنید و با ایجاد 3 یا چند بلوک از همان رنگ، امتیاز بگیرید.</p><p>زمان باقی مانده تا ناپدید شدن رگه ترکیبی شما با نشان داده می شود");
	if (gameState == 1) {
		pause();
	}

	if ($("#pauseBtn").attr('src') == baseUrl+"/resources/assets/hextris/images/btn_pause.svg" && gameState != 0 && !infobuttonfading) {
		return;
	}
